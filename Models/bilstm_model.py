...
from keras.layers import Embedding, Dropout, BatchNormalization, Bidirectional, LSTM, Dense
...
class BiLSTM_model:
    """
    This class defines an BiLSTM-based model for binary classification tasks.
    """

    def __init__(self, input_dim, output_dim, input_length):
        """
        This class defines a BiLSTM-based model for binary classification tasks.
        """
    ...
    def create_deep_model(self):
        """
        Initializes the BiLSTM_model with specified input dimension, output dimension, and input length.
        """
        model = Sequential()
        model.add(Embedding(input_dim=self.input_dim, output_dim=100, input_length=self.input_length))
        model.add(Dropout(0.3))
        model.add(Bidirectional(LSTM(32, return_sequences=True, kernel_regularizer=tf.keras.regularizers.l2(0.01))))
        model.add(Dropout(0.3))
        model.add(BatchNormalization())
        model.add(Bidirectional(LSTM(32, kernel_regularizer=tf.keras.regularizers.l2(0.01))))
        model.add(Dropout(0.3))
        model.add(BatchNormalization())
