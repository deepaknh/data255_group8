...
class LSTM_model:
    """
    This class defines an LSTM-based model for binary classification tasks.
    """

    def __init__(self, input_dim, output_dim, input_length):
        """
        Initializes the LSTM_model with specified input dimension, output dimension, and input length.
        """
    ...
    def create_deep_model(self):
        """
        Creates and configures a deep LSTM model for binary classification.
        """
        model = Sequential()
        model.add(Embedding(input_dim=self.input_dim, output_dim=100, input_length=self.input_length))
        model.add(Dropout(0.3))
        model.add(LSTM(32, return_sequences=True, kernel_regularizer=tf.keras.regularizers.l2(0.01)))
        model.add(Dropout(0.3))
